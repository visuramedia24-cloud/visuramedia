...
// Order: Utilities → Core Functions → Navigation → Forms → Lazy Loading → Init → Background Effects
...
function initFAQ() {
    ...
    faqData.forEach(({ element, question, icon }) => {
        ...
        const faqClickHandler = () => {
            ...
            if (!isActive) { element.classList.add('active'); if (icon) icon.textContent = '−'; }
        };
...
function initParallax() {
    if (cachedElements.parallaxElements.length === 0) return;
    
    // Disable parallax on mobile devices
    const isMobile = window.matchMedia('(max-width: 768px)').matches;
    if (isMobile) {
        // On mobile, just ensure elements are visible without transform
        cachedElements.parallaxElements.forEach((element) => {
    ...
    registerEventHandler(window, 'scroll', parallaxHandler, { passive: true });
}
...
// ====================================
// Floating Nav - Intersection Observer (Phase 1 optimization)
function initFloatingNav() {
    ...
    if ('IntersectionObserver' in window) {
        ...
        // Using Intersection Observer for floating nav
...
function initLuxuryNavigation() {
    ...
    if (!mobileMenuBtn || !mobileMenu || !closeMobileMenu || !mobileMenuOverlay) return;
    const openMenuHandler = function() { mobileMenu.classList.add('active'); mobileMenuOverlay.classList.add('active'); document.body.style.overflow = 'hidden'; };
    const closeMenuHandler = function() { mobileMenu.classList.remove('active'); mobileMenuOverlay.classList.remove('active'); document.body.style.overflow = ''; };
    registerEventHandler(mobileMenuBtn, 'click', openMenuHandler);
    registerEventHandler(closeMobileMenu, 'click', closeMenuHandler);
    registerEventHandler(mobileMenuOverlay, 'click', closeMenuHandler);
    const mobileLinks = mobileMenu.querySelectorAll('.luxury-nav-mobile-link');
    mobileLinks.forEach(link => { registerEventHandler(link, 'click', function() { mobileLinks.forEach(l => l.classList.remove('text-gold', 'font-semibold')); this.classList.add('text-gold', 'font-semibold'); closeMenuHandler(); }); });
}
...
// Navigation Highlight - Intersection Observer (Phase 1 optimization)
function initNavigationHighlight() {
    ...
    // Using Intersection Observer for navigation highlight
}
...
const validationMessages = {
    email: {
        ...
        patternMismatch: 'Bitte geben Sie eine gültige E-Mail-Adresse ein.',
...
function checkEmailJSStatus() {
    ...
    if (typeof emailjs === 'undefined') {
        console.error('❌ EmailJS library not loaded!');
        console.log('Check if the CDN script is loaded correctly.');
        return false;
    }
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                img.onerror = () => {
                    console.warn('Hero background image failed to load:', bgImageUrl);
                    observer.disconnect();
                };
