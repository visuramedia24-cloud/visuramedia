...
const PAGESPEED_API_KEY = 'AIzaSyDwSIFOClWccE5m1fEmMWfl-fXrIaOYmuU';
...
async function handleFormSubmit(e) {
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        ...
        console.log('Full PageSpeed Report:', report);
    }
...
async function fetchPageSpeedData(url, strategy = 'mobile') {
    try {
        ...
        console.log('Fetching PageSpeed data with ALL categories...');
        ...
        if (!response.ok) {
            if (response.status === 429) {
                ...
                console.error('PageSpeed API: Invalid URL');
                throw new Error('Invalid website URL provided.');
            } else if (response.status === 403) {
                console.error('PageSpeed API: API key invalid or quota exceeded');
        ...
        console.log('✅ Successfully fetched ALL PageSpeed categories');
        return data;
    } catch (error) {
...
function parsePageSpeedData(data) {
    ...
    console.log('📊 Parsed PageSpeed Metrics:', {
...
async function generateReport(data, pageSpeedData = null) {
    ...
    if (pageSpeedData) {
        ...
        console.log('✅ Using REAL PageSpeed data:', scores);
    } else {
        ...
        console.log('⚠️ Using SIMULATED data (API failed):', scores);
    }
...
function populateProblemSection(scores, metrics) {
    ...
    console.log('✅ Populated dynamic problem section:', {
        averageScore,
        performanceScore,
        targetScore,
        timeframe,
